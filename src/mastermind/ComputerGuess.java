...
public class ComputerGuess extends Guess{
	 ...
	 * @return none
	 * @effect generates a list of MAXDECIMALVALUE items that could be potential answers
	 * */
	private static void generatePotAns(){
		...
		for (int i=0;i<MAXDECIMALVALUE;i++){
			//generates the base 6 equivalent of the answer starting with i=0 abd inputInt = 1111
	 ...
	 * @param inputGuess computers guess
	 * @param comparedResult result from comparison to actual
	 * @return none
	 * @effect change the solutioin state of all invalid answers to 0
	 * */
	...
	private static int[] nextGuess(){
		...
		// loop through each potential guess to see which eliminates the most when eliminating the least amount
		for(int count=0;count<MAXDECIMALVALUE;count++){
			...
			//if the number eliminated is equal to the minimum number eliminated, keep the smaller guess
			if(potMin==minNumElim){
			}
			//otherwise, if the new potential minimum is greater than the current, replace it and the next guess
	 ...
	 * @param potGuess the potential guess that will be tested
	...
	private static int nextCondenseCounter(int[] potGuess){
		...
		int[][] responses= {{0,0},{0,1},{0,2},{0,3},{0,4},{1,0},{1,1},{1,2},{1,3},{2,0},{2,1},{2,2},{3,0},{4,0}};
		...
		for(int responsecounter=0;responsecounter<14;responsecounter++){
			...
			for(int potRespCheck=0;potRespCheck<MAXDECIMALVALUE;potRespCheck++){
				// if the solution is still possible and the compared guess does not equal the response increment counter
				if(solutionState[potRespCheck]==1 && (compareGuess(potGuess,potAns[potRespCheck])!=responses[responsecounter])){
					returnCounter++;
				}
			}
			// if the new counter is smaller than the one that will be returned, replace the one that will be returned
